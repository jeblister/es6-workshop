...
it.skip('takes a compare function, returns the index where it returned true', function() {
    const foundAt = [false, true].findIndex(item);

    assert.equal(foundAt, 1);
  });

it.skip('returns the first position it was found at', function() {
    const foundAt = [0, 1, 1, 1].findIndex(item => item = 1);

    assert.equal(foundAt, 1);
  });

it.skip('returns `-1` when nothing was found', function() {
    const foundAt = [1, 2, 3].findIndex(item => item > 1);

    assert.equal(foundAt, -1);
  });

it.skip('the findIndex callback gets the item, index and array as arguments', function() {
    const three = 3;
