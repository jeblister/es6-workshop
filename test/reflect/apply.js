...
describe('`Reflect.apply` calls a target function', function() {
  ...
  it.skip('it is a static method', function() {
    const expectedType = '???';

    assert.equal(typeof Reflect.apply, expectedType)
  });

  it.skip('it calls a callable, e.g. a function', function() {
    let fn;

    assert.equal(Reflect.apply(fn), 42);
  });

  it.skip('passing it a non-callable throws a TypeError', function() {
  ...
  it.skip('the second argument is the scope (or the `this`)', function() {
    class FourtyTwo {
  ...
  it.skip('the 3rd argument is an array of parameters passed to the call', function() {
